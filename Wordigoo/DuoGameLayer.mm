...
-(id)init{
    ...
    if(self){
        [self setCurrentPlayer:P1];
        
        [[WGSoundCore sharedDirector]playEffectSafeVolume:@"dundin.mp3"];
        
        orient_timeout_triggered=NO;
        //[self totalScoreforCurrentPlayer] totalScore=0;
        totalscoreP1_=0;
        totalscoreP2_=0;
...
-(void)actTouches:(NSSet*)touches
{
    ...
    for(b2Body *b=world->GetBodyList();b;b=b->GetNext())
    {
        ...
        if (([hx1 containsTouchLocation:touch1])&&b1)
        {
            ...
            //[self removeChild:hx1 cleanup:YES];
            //[hx1 removeFromParentAndCleanup:YES];
            //world->DestroyBody(b);
        }//end of if..
    ...
    for(b2Body *b=world->GetBodyList();b;b=b->GetNext())
    {
        ...
        if (([hx1 containsTouchLocation:touch1]))
        {
            ...
            else//clone modda zaten bir tane klonlanacak boncuk seçilmişse
            {
                ...
                if(self.totalScoreforCurrentPlayer >=100)
                    self.totalScoreforCurrentPlayer -=100;
                
                [self setScoreLabel:[NSString stringWithFormat:@"%d",[self totalScoreforCurrentPlayer]] forPlayer:[self CurrentPlayer]];
                ////////////////////
...
-(void)popAnimateAddedScore:(NSUInteger)newPoint
{
    ...
    CCMenuAdvanced *top_menu=(CCMenuAdvanced*)[self getChildByTag:TAG_DUO_SCORE_MENU_LAYER];
    
    CCLabelBMFont *score_lbl_=NULL;
    
    score_lbl_=([self CurrentPlayer]==P1)?
    (CCLabelBMFont*)[top_menu getChildByTag:TAG_DUO_SCORE_LABEL1]:
    (CCLabelBMFont*)[top_menu getChildByTag:TAG_DUO_SCORE_LABEL2];
    ...
    NSString *tmp_=[NSString stringWithFormat:@"%d",newPoint];
    int scaleFactor_=[tmp_ length]-1;
...
#pragma mark >Eğer bir kelime seçilmişse ve veritabanında varsa.
    if (isWordExists_)//!!!!seçilen kelime bulundu ise puanlama falan burda yap
    {
        ...
        int tot_=[self totalScoreforCurrentPlayer];
        tot_+=newPoint;
        
        [self setTotalScoreforCurrentPlayer:tot_];
...
-(void)initTest{
    ...
    //[[CCSpriteFrameCache sharedSpriteFrameCache]removeSpriteFramesFromFile:@"PhysicalSpritesObjects_Objects.plist"]];
    [[CCSpriteFrameCache sharedSpriteFrameCache]removeSpriteFrames];
    
    //[[CCSpriteFrameCache sharedSpriteFrameCache] addSpriteFramesWithFile:@"PhysicalSpritesObjects_Objects.plist"]];
    
    //NSString *plistfn_=[NSString stringWithFormat:@"PhysicalSpritesObjects_%@_Objects.plist",[WGThemeCore selectedThemeNameOnly]];
    //if ([[KTHardwareUtils getDeviceModel]isEqualToString:@"iPhone5"])
    //int a=[[CCDirector sharedDirector]winSize].height;
    if(IS_IPHONE5)
    {
...
-(void)generateHexagonAtPosition:(CGPoint)p withLetter:(NSString*)aletter
{
    ...
    if (([self totalScoreforCurrentPlayer]>0)&&(!aletter))
    {
        int tot_=[self totalScoreforCurrentPlayer];
        tot_-=2;
        [self setTotalScoreforCurrentPlayer:tot_];
        [self setScoreLabel:[NSString stringWithFormat:@"%d",tot_] forPlayer:[self CurrentPlayer]];
    }
...
{
    int ret_=([self CurrentPlayer]==P1)?totalscoreP1_:totalscoreP2_;
    return  ret_;
}
...
-(void) setTotalScoreforCurrentPlayer:(NSUInteger)totalscore
{
    if ([self CurrentPlayer]==P1)
        totalscoreP1_=totalscore;
    else
        totalscoreP2_=totalscore;
}

-(void)exchangeCurrentPlayer
{
    PlayerType P= ([self CurrentPlayer]==P1)?P2:P1;
    [self setCurrentPlayer:P];
    
    CCBoardLayer *score_layer_=(CCBoardLayer*)[self getChildByTag:TAG_DUO_SCORE_MENU_LAYER];
    CCLabelBMFont *lbl_P1_=(CCLabelBMFont*)[score_layer_ getChildByTag:TAG_DUO_SCORE_LABEL1];
    CCLabelBMFont *lbl_P2_=(CCLabelBMFont*)[score_layer_ getChildByTag:TAG_DUO_SCORE_LABEL2];
    
    
    if(P==P1)
    {
        [lbl_P1_ setColor:ccc3(255, 0, 0)];
        [lbl_P2_ setColor:ccc3(255, 255, 255)];
    }
    else
    {
        [lbl_P1_ setColor:ccc3(255, 255, 255)];
        [lbl_P2_ setColor:ccc3(255, 0, 0)];
    }
}
@end
